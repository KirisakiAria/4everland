<template>
    <section class="home">
        <section ref="app" class="canvas"></section>
        <section class="wrapper">
            <h1>4everland Is The Best</h1>
            <p>
...
<script>
            ...
            setClass() {
                class App {
                    ...
                    initPasses() {
                        ...
                        const smaaPass = new POSTPROCESSING.EffectPass(
                            this.camera,
                            new POSTPROCESSING.SMAAEffect(this.assets.smaa.search, this.assets.smaa.area, POSTPROCESSING.SMAAPreset.MEDIUM),
                        )
                ...
                class Road {
                    ...
                    createPlane(side, width, isRoad) {
                        const options = this.options
                        let segments = 100
                        const geometry = new THREE.PlaneBufferGeometry(isRoad ? options.roadWidth : options.islandWidth, options.length, 20, segments)
                        let uniforms = {
                            uTravelLength: new THREE.Uniform(options.length),
                            uColor: new THREE.Uniform(new THREE.Color(isRoad ? options.colors.roadColor : options.colors.islandColor)),
                            uTime: this.uTime,
                        }
                        if (isRoad) {
                    ...
                    getDistortion: `
                     ...
                     vec3 getDistortion(float progress){
                             ...
                             return vec3(
                                         ...
                                         nsin(progress * PI * uFreq.y + uTime) * uAmp.y - nsi(movementProgressFix * PI * uFreq.y + uTime) * uAmp.y,
                                     nsin(progress * PI * uFreq.z + uTime) * uAmp.z - nsi(movementProgressFix * PI * uFreq.z + uTime) * uAmp.z
                                 );
                         }
                         `,
                    getJS: (progress, time) => {
    ...
    `,
                ...
                this.init(App, turbulentDistortion)
            },
            init(App, turbulentDistortion) {
                const options = {
                    onSpeedUp: ev => {},
                    onSlowDown: ev => {},
                ...
                const myApp = new App(this.$refs.app, options)
